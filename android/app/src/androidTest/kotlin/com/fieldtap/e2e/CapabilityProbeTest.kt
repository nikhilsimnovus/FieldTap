 ...
 *     and self-consistent — never POSSIBLE, because this image genuinely has no `/dev/diag` (proved by a direct
 ...
 *
 * The test gains no root and runs no exploit; it stats `/dev/diag` only to record the device's real state. No
 * identifier is read anywhere.
 */
...
class CapabilityProbeTest {
    ...
    fun detectsCapabilityRootDiagAndUsbDebuggingHonestly() {
        ...
        // What the device really is, read directly, so the honesty checks rest on facts, not on the app. ls -l
        // /dev/diag reports "No such file" on stderr, which executeShellCommand does not capture, so the file is
        // stated directly (absent => exists() is false); the ls stdout is kept only for the record.
        val diagLs = E2e.shell("ls -l /dev/diag").trim()
        val diagAbsentOnDevice = !File("/dev/diag").exists()
        ...
        // Write the artifacts first, so a later assertion failure still leaves the pulled JSON to diagnose it.
        ...
        File(E2e.outputDir(), CAPABILITY_JSON).writeText(CapabilityJson.encode(report), Charsets.UTF_8)
        E2e.writeResult(
