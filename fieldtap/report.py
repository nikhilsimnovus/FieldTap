...
def render_html(summary: dict, meta: dict, events: list, kpi_rows: list, track, traffic_results: list,
                cells: list, ladder: str, flow_events: list, started: Optional[datetime], kpi_note: str) -> str:
    ...
    if kpi_rows and started:
        ...
        if series:
            charts.append("<h3>RSRP over time (from MeasurementReports)</h3>" +
...
<section><h2>Radio</h2>%(charts)s</section>
<section><h2>Procedures</h2>%(procs)s</section>
<section><h2>Events</h2>%(events)s</section>
<section><h2>Serving cells</h2>%(cells)s</section>
<section><h2>Traffic tests</h2>%(traffic)s</section>
<section><h2>Call flow</h2><pre>%(ladder)s</pre></section>
</main><footer>Generated by FieldTap %(version)s. Decode by Wireshark; open capture.pcapng for the full message detail.</footer></body></html>
""" % {"title": _esc(title), "css": _CSS, "sub": _esc(" | ".join(sub)), "tiles": "".join(tiles), "kv": "".join(kv),
       ...
       "charts": charts_html, "procs": proc_html, "events": events_html, "cells": cells_html, "traffic": traffic_html,
       "ladder": _esc(ladder or "(no messages)"), "version": _esc(__version__)}
